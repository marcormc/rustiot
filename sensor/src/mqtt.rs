...
pub fn start_mqtt_client(_tx: &mpsc::Sender<Event>) -> Result<EspMqttClient, EspError> {
    ...
    // let payload: &[u8] = &[];
    client.publish("/rust/test", QoS::AtLeastOnce, true, b"greetings from rust node.")?;
...
fn process_message(message: &EspMqttMessage) {
    match message.details() {
        Complete => {
            ...
            info!("Mensaje mqtt: {:?}, data: {:?}", message, message.data());
